...
expid=st
torchrun --standalone --nnodes 1 --nproc_per_node 4 main.py exp_id=${expid} batch_size=20 data=fluids_sample compile=1 optim=muon model=time_then_space &&
...
torchrun --standalone --nnodes 1 --nproc_per_node 2 main.py exp_id=${expid} batch_size=80 data=fluids_sample compile=1 optim=muon model.patch_num=8 &&
...
torchrun --standalone --nnodes 1 --nproc_per_node 2 main.py exp_id=${expid} batch_size=80 data=fluids_sample compile=1 optim=muon model.patch_num=4 &&
