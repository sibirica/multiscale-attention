...
def _alignment(tokens: torch.Tensor, time_emb: torch.Tensor, spatial_tokens: int) -> float:
    ...
    if time_emb.dim() == 3:
        time_emb = time_emb.squeeze(1)
    if time_emb.size(-1) != tokens.size(-1):
        time_emb = time_emb[:, : tokens.size(-1)]
...
def _alignment_with_offset(tokens: torch.Tensor, time_emb: torch.Tensor, spatial_tokens: int, offset: int) -> float:
    total = tokens.size(1)
    if total % spatial_tokens != 0:
        return float("nan")
    time_emb = time_emb.squeeze(0)
    ...
    pred_time = sims.argmax(dim=-1)
    t_major = torch.arange(total) // spatial_tokens
    t_major = torch.clamp(t_major + offset, min=0, max=time_emb.size(0) - 1)
    return (pred_time == t_major).float().mean().item()
...
def main() -> None:
    ...
    def _layer_hook(layer, inputs, _outputs):
        x_slow = inputs[1]
        ...
        if y_fast.size(1) > inputs[0].size(1):
            y_fast = y_fast[:, : inputs[0].size(1)]
        base = _alignment(y_fast.detach().cpu(), time_emb_slow, spatial_tokens)
        shifted = _alignment_with_offset(y_fast.detach().cpu(), time_emb_slow, spatial_tokens, offset=-1)
        lift_scores.append((base, shifted))
    ...
    for i, (base, shifted) in enumerate(lift_scores):
        print(f"layer_{i}: lift t-major={base:.3f}, offset-1={shifted:.3f}")

    print("Layer lift alignment trace complete.")
