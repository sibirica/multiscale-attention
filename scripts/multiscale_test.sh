...
# torchrun --standalone --nnodes 1 --nproc_per_node 4 src/main.py eval_only=1 use_wandb=0 exp_name=eval eval_from_exp=checkpoint/multiscale/${expid} log_eval_plots=-1 exp_id=${expid} data=fluids_bench batch_size_eval=64 model=multiscale_bcat model.flex_attn=1
# torchrun --standalone --nnodes 1 --nproc_per_node 4 src/main.py eval_only=1 use_wandb=0 exp_name=eval eval_from_exp=checkpoint/multiscale/${expid} log_eval_plots=-1 exp_id=${expid} data=fluids_bench batch_size_eval=64 model=multiscale_bcat model.flex_attn=1 overfit_test=1

# vanilla bcat
expid=bcat_baseline_8
...
torchrun --standalone --nnodes 1 --nproc_per_node 4 src/main.py eval_only=1 use_wandb=0 exp_name=eval eval_from_exp=checkpoint/multiscale/${expid} log_eval_plots=-1 exp_id=${expid} data=fluids_bench batch_size_eval=64 model=bcat model.flex_attn=1
torchrun --standalone --nnodes 1 --nproc_per_node 4 src/main.py eval_only=1 use_wandb=0 exp_name=eval eval_from_exp=checkpoint/multiscale/${expid} log_eval_plots=-1 exp_id=${expid} data=fluids_bench batch_size_eval=64 model=bcat model.flex_attn=1 overfit_test=1
