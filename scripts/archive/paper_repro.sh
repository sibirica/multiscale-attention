...
do
    ...
    torchrun --standalone --nnodes 1 --nproc_per_node 2 src/main.py eval_only=1 use_wandb=0 exp_name=fluids_eval reload_model=checkpoint/fluids_test/${expid} log_eval_plots=-1 exp_id=${expid} batch_size_eval=32 model=fno amp=0 data=${ds}
done
...
do
    ...
    CUDA_VISIBLE_DEVICES=$GPU python src/main.py eval_only=1 use_wandb=0 exp_name=fluids_eval reload_model=checkpoint/fluids_test/${expid} log_eval_plots=-1 exp_id=${expid} batch_size_eval=128 model=unet data=${ds}
done
...
do
    ...
    CUDA_VISIBLE_DEVICES=$GPU python src/main.py eval_only=1 use_wandb=0 exp_name=fluids_eval reload_model=checkpoint/fluids_test/${expid} log_eval_plots=-1 exp_id=${expid} batch_size_eval=128 model=deeponet data=${ds}
done
